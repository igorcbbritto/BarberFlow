/**
 * layout.js
 * Injeta o sidebar e header compartilhado em todas as páginas do dashboard.
 * Chame initLayout() no início de cada página.
 */
...
function initLayout(activePage) {
      ...
      <a href="${item.href}"
         class="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all
                ${isActive
                  ...
                  : 'text-gray-400 hover:text-white hover:bg-dark-4'}">
        ${item.icon}
  ...
  document.getElementById('app-layout').innerHTML = `
    <div class="flex min-h-screen" style="background:#0D0D0D">

      <!-- ── SIDEBAR ── -->
      <aside class="w-64 flex-shrink-0 flex flex-col fixed inset-y-0 left-0 z-30"
             style="background:#111; border-right:1px solid #1e1e1e;">
        ...
        <div class="px-4 py-4 border-t" style="border-color:#1e1e1e;">
          <div class="flex items-center gap-3">
            <div class="w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold"
                 style="background:rgba(201,168,76,0.2); color:#C9A84C;" id="user-avatar">J</div>
      ...
      <main class="flex-1 ml-64 flex flex-col min-h-screen">
        <div id="page-content" class="flex-1 p-8"></div>
      </main>
    </div>
  `;
...
function scissorsIcon() {
  return `<svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    <circle cx="6" cy="6" r="3" stroke-width="2"/><circle cx="6" cy="18" r="3" stroke-width="2"/>
    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
          d="M20 4L8.12 15.88M14.47 14.48L20 20M8.12 8.12L12 12"/>
  </svg>`;
}

// CSS compartilhado injetado no head
